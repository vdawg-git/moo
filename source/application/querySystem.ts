...
export type QuerySystem = {
	readonly useQuery: {
		<T>(
			key: string | string[],
			query: () => Promise<Result<T, any>>
		): QueryResult<T>
		<T>(key: string | string[], query: () => Promise<T>): QueryResult<T>
	}
	readonly observeQuery: <T>(
		key: JsonValue,
		query: () => Promise<Result<T, any>> | Promise<T>
	) => Observable<QueryResult<T>>
}

/** Creates a query system bound to the given database's change stream */
export function createQuerySystem(changed$: Observable<string>): QuerySystem {
	...
	function observeQuery<T>(
		...
		const query$ = changed$.pipe(switchMap(() => query().catch(Result.error)))
		const stream$ = merge(
			query$,
	...
	function useQuery<T>(
		...
		useEffect(() => {
			const subscription = observeQuery(combinedKey, callback).subscribe(
				setState
			)
